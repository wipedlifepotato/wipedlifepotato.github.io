...
func testFM() {
	...
	r = mRtl.Rtl{}
	if useRTLTCP {
		r.ConnectSDR(mRTLTCP.Host, mRTLTCP.Port, Hz, 32.0)
	} else {
		r.InitSDR(0, Hz, 32.0)
	}
	win = waterfall.Win{}
	...
	for win.Running {
		...
		*/
		...
		if Hz != win.Hz {
			...
			Hz = win.Hz
			if useRTLTCP {
				r.ReConnectSDR(mRTLTCP.Host, mRTLTCP.Port, Hz, 32.0)
			} else {
				r.ReInitSDR(0, Hz)
			}
		}
...
func main() {
	...
	flag.Bool("usetcp", false, "use rtl-tcp instead hardware")
	pflag.CommandLine.AddGoFlagSet(flag.CommandLine) // an another
	flag.Int("rtltcpport", 1234, "port for rtl-tcp")
	pflag.CommandLine.AddGoFlagSet(flag.CommandLine) // an another
	flag.String("rtltcphost", "localhost", "use rtl-tcp host")
	pflag.CommandLine.AddGoFlagSet(flag.CommandLine) // an another
	pflag.Parse()
	viper.BindPFlags(pflag.CommandLine)

	fmt.Println(viper.GetInt("rtltcpport"))
	if viper.GetBool("usetcp") {
		useRTLTCP = true
		h := viper.GetString("rtltcphost")
		p := viper.GetInt("rtltcpport")
		mRTLTCP = rtltcp{h, p}
		fmt.Println("use rtl-tcp")
	} else {
		fmt.Println("use hardrware")
	}
